...
create table regions(region_id int not null,region_name varchar(50) not null,primary key(region_id));
create table countries(country_id varchar(10) not null,country_name varchar(50),region_id int,primary key(country_id),foreign key(region_id) references regions(region_id));
create table locations(location_id int not null,street_address varchar(70),postal_code varchar(20),city varchar(50),state_province varchar(25),country_id varchar(10),primary key(location_id),foreign key(country_id) references countries(country_id));
create table departments(department_id int not null,department_name varchar(50),location_id int,primary key(department_id),foreign key(location_id) references locations(location_id));
create table jobs(job_id int primary key not null,job_title varchar(50),min_salary float,max_salary float);
create table employees(employee_id int primary key not null,first_name varchar(30) not null,last_name varchar(20),email varchar(50),phone_number varchar(15),hire_date date,job_id int,salary int,manager_id int,department_id int,foreign key(job_id)references jobs(job_id),foreign key(department_id) references departments(department_id));
create table dependents(dependent_id int not null,first_name varchar(20) not null,last_name varchar(10),relationship varchar(20),employee_id int,primary key(dependent_id),foreign key(employee_id) references employees(employee_id));
...
/* Modify Column Salary from int to smallint */

alter table employees modify salary smallint;
describe employees;

/* Add commission column to employees table */
